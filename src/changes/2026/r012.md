...
- varnish-7.x is still default for the `webproxy` role but has known vulnerabilities. Consider updating to varnish-8.0 by setting `services.varnish.package = pkgs.varnish80;`. Note the [breaking changes](https://vinyl-cache.org/docs/8.0/whats-new/upgrading-8.0.html) of that updated.
